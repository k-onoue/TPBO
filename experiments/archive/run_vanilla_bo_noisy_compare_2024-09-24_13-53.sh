            ...
            for SEED in {0..9}; do
                for NOISE_VAL in "${NOISE[@]}"; do
                    ...
                    sbatch --job-name="${EXPERIMENT_NAME}" \
                           ...
                           --partition=$PARTITION \
                           --time=$TIME \
                           --wrap="python3 experiments/2024-09-23/vanilla_bo_compare.py --seed $SEED --objective $OBJECTIVE --noise_strength $NOISE_VAL --acquisition $ACQUISITION --surrogate $SURROGATE --iterations $ITER"
                done
            done
        done
    done
