                ...
                for NOISE_TYPE in "${NOISE_TYPES[@]}"; do
                    for NOISE_VAL in "${NOISE_STRENGTH[@]}"; do
                        ...
                        sbatch --job-name="${EXPERIMENT_NAME}" \
                               ...
                               --partition=$PARTITION \
                               --time=$TIME \
                               --wrap="python3 experiments/2024-09-24/vanilla_bo.py --seed $SEED --objective $OBJECTIVE --noise_type $NOISE_TYPE --noise_strength $NOISE_VAL --acquisition $ACQUISITION --surrogate $SURROGATE --iterations $ITER"
                    done
                done
            done
        done
